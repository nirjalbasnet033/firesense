...
export default function SignUpForm() {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-4 py-24 relative overflow-hidden">
      ...
      {/* Atmospheric glows */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/8 rounded-full blur-[150px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-accent/6 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="glass-card-dark rounded-3xl p-8 sm:p-10 border border-border animate-border-glow">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="font-extrabold text-2xl text-foreground mb-2">Create Account</h1>
            <p className="text-sm text-muted-foreground">
              Join 52,000+ players. Get your first config today.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <div className="relative">
                <Icon name="UserIcon" size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
                <input
                  type="text"
                  placeholder="your_gamertag"
                  value={formData.username}
                  onChange={(e) => setFormData({ ...formData, username: e.target.value })}
                  className={`${inputClass('username')} pl-10 font-mono`}
                />
              </div>
              {errors.username && (
                <p className="mt-1.5 text-xs text-red-400 font-mono">{errors.username}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.confirmPassword && (
                <p className="mt-1.5 text-xs text-red-400 font-mono">{errors.confirmPassword}</p>
              )}
            </div>
            ...
            <div>
              <label className="flex items-start gap-3 cursor-pointer group">
                <div className="relative mt-0.5 shrink-0">
                  ...
                  >
                    {formData.agree && <Icon name="CheckIcon" size={12} className="text-white" />}
                  </div>
                </div>
                <span className="text-xs text-muted-foreground leading-relaxed">
                  I agree to Civilian Sensi{' '}
              ...
              {errors.agree && (
                <p className="mt-1.5 text-xs text-red-400 font-mono">{errors.agree}</p>
              )}
            </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-6">
          {[
            ...
            { icon: 'BoltIcon', text: 'Instant Access' },
          ].map((b) => (
